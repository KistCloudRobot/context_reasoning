...
obstruct(Agent1, Agent2):-
	inFrontOf(Agent1, Agent2),
	near(Agent1, Agent2).
	
faceWith(Agent1, Agent2):-
	inFrontOf(Agent2, Agent1),
	inFrontOf(Agent1, Agent2),
	near(Agent1, Agent2).
	
onBoxError(Box, Error):-
    type(Box, 'http://knowrob.org/kb/knowrob.owl#Box'),
	near(Robot, Box),
	not(on_Physical(Box, Robot)),
	on_Physical(Box, 'http://www.arbi.com/arbi#floor'),
	Error = 'fallen'.

inFrontOf(Front, Back):-
	type(Front, 'http://knowrob.org/kb/knowrob.owl#Robot'),
	type(Back, 'http://knowrob.org/kb/knowrob.owl#Robot'),
	Front\=Back,
	near(Front,Back),
	not(contains('http://www.arbi.com/arbi#chargingArea01', Front)),
	not(contains('http://www.arbi.com/arbi#chargingArea02', Front)),
	not(contains('http://www.arbi.com/arbi#chargingArea03', Front)),
	not(contains('http://www.arbi.com/arbi#chargingArea04', Front)),
	not(contains('http://www.arbi.com/arbi#chargingArea01', Back)),
	not(contains('http://www.arbi.com/arbi#chargingArea02', Back)),
	not(contains('http://www.arbi.com/arbi#chargingArea03', Back)),
	not(contains('http://www.arbi.com/arbi#chargingArea04', Back)).
	
pre_currentObjectPerception(Object, CurrentPerception):-
   (rdf(Object, rdf:type, knowrob:'Artifact');rdf(Object, rdf:type, knowrob:'Box');rdf(Object, rdf:type, knowrob:'Rack')),
...
currentObjectPerception(Object, CurrentPerception):-
   (rdfs_individual_of(Object,  knowrob:'Artifact');rdfs_individual_of(Object,  knowrob:'Box');rdfs_individual_of(Object,  knowrob:'Rack')),
...
semanticMapPerception(Object, Perception):-
   ...
   (rdfs_individual_of(Object, knowrob:'Area');rdfs_individual_of(Object, knowrob:'Artifact')),
...
objectSize(Object, Size):-
   (type(Object, 'http://knowrob.org/kb/knowrob.owl#Artifact');type(Object,'http://knowrob.org/kb/knowrob.owl#Rack');type(Object,'http://knowrob.org/kb/knowrob.owl#Box')),
   not(rdfs_individual_of(Object,  knowrob:'Hand')),
...
areaSize(Area, Size):-
   rdfs_individual_of(Area,  'http://knowrob.org/kb/knowrob.owl#Area'),
   rdf(Area, knowrob:depthOfObject, literal(type(_,S1x))),atom_to_term(S1x,S1X,_),
   rdf(Area, knowrob:widthOfObject, literal(type(_,S1y))),atom_to_term(S1y,S1Y,_),
   rdf(Area, knowrob:heightOfObject, literal(type(_,S1z))),atom_to_term(S1z,S1Z,_),
   Size = [S1X, S1Y, S1Z].
...
areaPose(Area, Pose):-
   semanticMapPerception(Area, SemanticMapPerception),
   type(Area, 'http://knowrob.org/kb/knowrob.owl#Area'),
   rdf(SemanticMapPerception, knowrob:eventOccursAt, Object_Matrix),
   rdf(Object_Matrix,'http://knowrob.org/kb/knowrob.owl#m03',literal(type(_,P1x))),atom_to_term(P1x,P1X,_),
   rdf(Object_Matrix,'http://knowrob.org/kb/knowrob.owl#m13',literal(type(_,P1y))),atom_to_term(P1y,P1Y,_),
   rdf(Object_Matrix,'http://knowrob.org/kb/knowrob.owl#m23',literal(type(_,P1z))),atom_to_term(P1z,P1Z,_),
...
contains(Area, Entity):-
	type(Area, 'http://knowrob.org/kb/knowrob.owl#Area'),
	not(Area == 'http://www.arbi.com/arbi#floor'),
	once(
	(type(Entity, 'http://knowrob.org/kb/knowrob.owl#Robot'), currentRobotBodyPose(Entity, EntityPose));
	(type(Entity, 'http://knowrob.org/kb/knowrob.owl#Box'), currentObjectPose(Entity, EntityPose));
	(type(Entity, 'http://knowrob.org/kb/knowrob.owl#Rack'), currentObjectPose(Entity, EntityPose))),

	areaPose(Area, AreaPose),
	areaSize(Area, AreaSize),
	
	nth0(0, AreaPose, AreaX),
	nth0(1, AreaPose, AreaY),
	
	nth0(0, EntityPose, EntityX),
	nth0(1, EntityPose, EntityY),

	nth0(0, AreaSize, AreaD),
	nth0(1, AreaSize, AreaW),

	((AreaX + AreaW*0.5 + 0.1) >  EntityX, (AreaX - AreaW*0.5 - 0.1) <  EntityX),
	((AreaY + AreaD*0.5 + 0.1) >  EntityY, (AreaY - AreaD*0.5 - 0.1) <  EntityY).
...
%추가 및 수정
currentAgentVertex(Agent, Vertice):-
   currentRobotBodyPose(Agent, AgentPose),
   robotBodySize(Agent, AgentSize),
   vertex(AgentPose, AgentSize, Vertice).
