...
:- get_file_path("cmProlog/prolog/init_s", Path), sub_string(Path,0,_,19,Dir), assert(root_directory_path(Dir)).
...
:- use_module(library('prolog/predicate_vrep')).
...
:- rdf_load(library(owl/vrep_model_skku_20200423)).
:- rdf_load(library(owl/test_object_skku2)).
